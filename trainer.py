...
class Trainer:
    ...
    def train(self, model_path: Optional[str] = None, trial: Union["optuna.Trial", Dict[str, Any]] = None):
        ...
        # isinstance(instance, class or data type) -> True or False
        # whether dataset is sized or not
        ...
        if train_dataset_is_sized: # True
            num_update_steps_per_epoch = len(train_dataloader) // self.args.gradient_accumulation_steps # 267 // 1
            num_update_steps_per_epoch = max(num_update_steps_per_epoch, 1) # 267
            # self.args.max_steps: -1
            # max_steps = 801
            if self.args.max_steps > 0:
                max_steps = self.args.max_steps # 801
                num_train_epochs = self.args.max_steps // num_update_steps_per_epoch + int(
                    self.args.max_steps % num_update_steps_per_epoch > 0
                ) # 801 // 267 + (801 % 267) = 3 -> 801 % 3 == 0
            else: # -1
                # math.ceil: Round up
        ...
        """
        return: max_steps,: 801 
                num_train_epochs,: 3 
                num_update_steps_per_epoch: 267 -> 267 * 3 = 801
        """
        ...
        # Train!
        if is_torch_tpu_available():
            total_train_batch_size = self.args.train_batch_size * xm.xrt_world_size()# 32
        else:
            total_train_batch_size = (
                self.args.train_batch_size # 32
                * self.args.gradient_accumulation_steps # 1
                * (torch.distributed.get_world_size() if self.args.local_rank != -1 else 1) # 1
            )
        ...
        logger.info("  Num examples = %d", num_examples) # 8540
        logger.info("  Num Epochs = %d", num_train_epochs) # 3
        logger.info("  Instantaneous batch size per device = %d", self.args.per_device_train_batch_size) # 4
        logger.info("  Total train batch size (w. parallel, distributed & accumulation) = %d", total_train_batch_size) # 32
        logger.info("  Gradient Accumulation steps = %d", self.args.gradient_accumulation_steps) # 1
        logger.info("  Total optimization steps = %d", max_steps) # 801
        ...
        self.state.trial_name = self.hp_name(trial) if self.hp_name is not None else None # None
        self.state.trial_params = hp_params(trial) if trial is not None else None # None
        ...
        # Whether or not this process is the global main process (when training in a distributed fashion
        # on several machines, this is only going to be :obj:`True` for one process).
        ...
        for epoch in range(epochs_trained, num_train_epochs):
            ...
            if is_torch_tpu_available():
                ...
                epoch_iterator = train_dataloader # epoch_iterator from train_dataloader
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                # It means the last step in epoch
                ...
                floating point operations for every backward + forward pass. 
                If using another model, either implement such a method in the model or subclass and override this method.
                """
                # e.g. )self._total_flos = 10765693550592
                ...
                if (step + 1) % self.args.gradient_accumulation_steps == 0 or (
                    ...
                    # scaling optimizer and normalizing parameters
                    if self.args.fp16 and _use_native_amp:
                        """
                    ...
                    # lr_scheduler step & zero grad & global step += 1
